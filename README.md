...
2. **제한 N건 판정**은 실행당 최대 100건, 레포당 최대 20건을 판정한다. **전체 N건 판정 (상한 없음)**은 수집한 이슈를 모두 판정한다. 두 버튼 모두 저비용 모델 `gpt-5.6-luna`, 추론 `low`, 최대 16개 동시 실행을 사용한다.
...
전체 모드는 두 건수 상한을 모두 해제하며 입력 순서로 전체를 판정한다. 기존 결과도 재판정하고 판정 파일을 교체한다.
