...
1. 레포 목록과 최근 갱신 기간을 선택하고 **이슈 수집**을 누른다. 이전에 저장한 목록은 **기본 100개 불러오기**로 교체할 수 있다. 기존 표본은 **고정 표본 30건 수집**으로 조회한다.
2. 수집된 건수를 확인하고 **수집한 이슈 판정**을 누른다. 기본 저비용 모델 `gpt-5.6-luna`, 추론 `low`로 실행한다.
3. 추천 후보, 전체, 제외, 미판정·실패 항목을 골라 판정 이유와 GitHub 이슈를 확인한다. 교차표, 레포 집중도, 사람 판정 비교도 펼쳐 볼 수 있다.
...
검증: `python3 test_web.py`, `python3 test_radar.py`.
...
기본은 **10개 분야 × 10개, 총 100개**다. AI 에이전트, 모델 추론, 프런트엔드, 백엔드,
데이터, 개발 도구, 인프라·보안, 생산성, 모바일·데스크톱, 미디어를 고르게 담았다.
2026-09-10 확인한 GitHub Trending에서 64개를 관찰했고, 나머지는 분야 균형을 위해 보완했다.
58개가 2023년 이후 생성됐으며, 100개 모두 최근 30일 내 push와 열린 이슈가 있다.
개별 레포와 조회 근거는 [선정 문서](docs/레포-선정.md)에 기록했다. 목록은 수동 선정이며 자동 갱신하지 않는다.

웹은 한 번에 최대 100개를 받는다. 전체 수집은 레포별 모든 페이지를 읽으므로 먼저 최근 1일로
