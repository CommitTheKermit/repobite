...
2. 화면의 판정 대상 건수를 확인하고 **수집한 이슈 판정**을 누른다. 실행당 최대 100건, 레포당 최대 20건을 저비용 모델 `gpt-5.6-luna`, 추론 `low`로 판정한다.
...
판정은 웹·CLI 모두 **실행당 최대 100건, 레포당 최대 20건**이다. 수집 파일에 등장한 레포 순서대로
한 건씩 번갈아 선택하고, 각 레포 안에서는 `created_at`이 최근인 이슈를 먼저 선택한다.
같은 생성 시각이면 입력 순서를 유지한다. 대소문자만 다른 레포 이름도 동일한 레포로 묶는다.
실패한 호출도 상한에 포함하며, 다른 이슈로 보충하거나 자동 재시도하지 않는다.
수집 원본은 유지하고 선택된 이슈의 성공·실패만 판정 파일에 저장한다. 상한 밖의 이슈는 웹에서 미판정으로 남는다.
리포트 비율은 선택된 판정 결과 기준이며 전체 수집분의 비율로 해석하면 안 된다.
이 상한은 한 번의 실행 기준이다. 기존 판정 재사용·일일 누적 한도는 아직 없으므로 다시 실행하면 같은 이슈도 재판정한다.
